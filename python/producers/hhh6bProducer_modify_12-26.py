...
from PhysicsTools.NanoNN.helpers.utils import closest, sumP4, polarP4, configLogger, get_subjets, deltaPhi, deltaR, get_mini_chi2
...
class hhh6bProducer(Module):
    ...
    def beginFile(self, inputFile, outputFile, inputTree, wrappedOutputTree):
        ...
        self.reader.BookMVA("bdt","/afs/cern.ch/user/r/rtu/CMSSW_11_1_0_pre5_PY3/src/PhysicsTools/NanoAODTools/condor/bdt-xml/TMVAClassification_BDT.weights.xml")
        '''
    ...
    def GetGenMatch_ofTau(self, event, looseTau):
        ...
        if higgs3_tau_match:
            self.out.fillBranch("higgs3_mass",(loosetaus_4vec[truth_trco_tau[0]]+loosetaus_4vec[truth_trco_tau[1]]).M())
        else:
    ...
    def selectLeptons(self, event):
        ...
        for tau in taus:
            ...
            # if tau.pt > 20 and abs(tau.eta) <= 2.3 and abs(tau.dz) < 0.2 and tau.idDeepTau2017v2p1VSe >= 2 and tau.idDeepTau2017v2p1VSmu >= 2 and tau.idDeepTau2017v2p1VSjet >= 8:
            if tau.pt > 20 and abs(tau.eta) <= 2.3 and abs(tau.dz) < 0.2 and tau.idDeepTau2017v2p1VSe >= 1 and tau.idDeepTau2017v2p1VSmu >= 1:
    ...
    def correctJetsAndMET(self, event):
        ...
        self.nSmallJets = int(len(event.ak4jets_PTcut30))
    ...
    def fillJetInfo(self, event, jets):
        ...
        for i in range(len(jets)):
            ...
            if self.isMC:
                ...
                jets_4vec[i].genJetIdx = jets[i].genJetIdx
        if self.isMC:
        ...
        if len(jets_4vec) > 5:
            ...
            self.out.fillBranch("hhh_resolved_mass", 0)
            self.out.fillBranch("hhh_resolved_pt", 0)
            ...
            for i_stop in range(20):
                stop_switch = True
                higgs1_hadronFlavour_chieck = (len(higgs1_minichi2) == 4)        #chieck if the flavour opposing
                higgs2_hadronFlavour_chieck = (len(higgs2_minichi2) == 4)
                if higgs1_hadronFlavour_chieck or higgs2_hadronFlavour_chieck:
                    stop_switch = False
                    if higgs1_hadronFlavour_chieck and (not higgs2_hadronFlavour_chieck):       # if higgs1 same flavour but higgs2 not, just remove higgs1 jet, which overlaps with higgs2 jet
                        for candi_higgs1 in [higgs1_minichi2[0],higgs1_minichi2[2]]:            # in the case len(higgs1_minichi2)==4, len(higgs2_minichi2)==2
                            if candi_higgs1 in higgs2_minichi2[0]:
                                higgs1_candi_jetlist.remove(candi_higgs1)
                        if len(higgs1_candi_jetlist) == 2:                                      # if not overlap, just remove higgs1_minichi2[2]
                            higgs1_candi_jetlist.remove(higgs1_minichi2[2])
                    elif higgs2_hadronFlavour_chieck and (not higgs1_hadronFlavour_chieck):
                        for candi_higgs2 in [higgs2_minichi2[0],higgs2_minichi2[2]]:            # in the case len(higgs1_minichi2)==2, len(higgs2_minichi2)==4
                            if candi_higgs2 in higgs1_minichi2[0]:
                                higgs1_candi_jetlist.remove(candi_higgs2)
                        if len(higgs2_candi_jetlist) == 2:
                            higgs2_candi_jetlist.remove(higgs2_minichi2[2])
                    else:
                        if higgs1_minichi2[0] == higgs2_minichi2[0]:
                            if higgs1_minichi2[1] < higgs2_minichi2[1]:
                                higgs2_candi_jetlist.remove(higgs2_minichi2[0])
                                higgs1_candi_jetlist.remove(higgs1_minichi2[2])
                            else:
                                higgs1_candi_jetlist.remove(higgs1_minichi2[0])
                                higgs2_candi_jetlist.remove(higgs2_minichi2[2])
                        else:
                            higgs1_candi_jetlist.remove(higgs1_minichi2[2])
                            higgs2_candi_jetlist.remove(higgs2_minichi2[2])
    ...
    def analyze(self, event):
        ...
        # HLT_conditions = Object(event, "HLT")
        # fill histograms
        event.gweight = 1
        if self.isMC:
            #if not HLT_conditions.PFHT300PT30_QuadPFJet_75_60_45_40_TriplePFBTagCSV_3p0:
            #    return False
        ...
        if self._opts['option'] == "5":
            ...
            # if (self.nSmallJets >= 4 and self.nBTaggedJets > 2 and self.nrawTaus > 0 ): passSel = True
            if (self.nSmallJets >= 4 and self.nrawTaus > 0 and self.nBTaggedJets > 2): passSel = True
